    ...
    return (
        <div className = "flex bg-gray-100 md:flex-row flex-col h-screen transition-height duration-75 ease-out" >
            <div className="hidden md:flex h-screen flex-initial">
                <Sideabar user={user && user} />     {/* Mobile Sidebar */}
            </div>
             
             <div className="flex md:hidden flex-row">
                 <div className="p-2 w-full flex flex-row justify-between items-center shadow-md ">

                <HiMenu fontSize={45} className="cursor-pointer" onClick={() => setToogleSidebar(true) } />
             <Link to="/">
             ...
             {toogleSidebar && (
                 <div className="fixed w-4/5 bg-white h-screen overflow-y-auto shadow-md z-10 animate-slide-in">
                     ...
                     <Sideabar user={user && user} closeToggle={setToogleSidebar} />     {/* Desktop Sidebar */}
                 </div>
             )}
             </div>
